...
import { mapChamberToAsset } from '../services/migrationService'

interface AssetsState {
  assets: Asset[]
  loading: boolean
  error: string | null
  fallbackSource: 'assets' | 'chambers'
}
...
export const fetchAssetsByType = createAsyncThunk<Asset[], AssetType, { rejectValue: string }>(
  ...
  async (type, { rejectWithValue, dispatch }) => {
    try {
      ...
      if (type === 'chamber' && assets.length === 0) {
        try {
          const chambers = await chamberService.getAllChambers()
          if (chambers.length > 0) {
            dispatch(assetsSlice.actions.setFallbackSource('chambers'))
            return chambers.map((c) => mapChamberToAsset(c))
          }
        } catch {
          dispatch(assetsSlice.actions.setFallbackSource('assets'))
        }
      }
      dispatch(assetsSlice.actions.setFallbackSource('assets'))
      return assets
...
const assetsSlice = createSlice({
  ...
  initialState,
  reducers: {
    setFallbackSource(state, action: PayloadAction<'assets' | 'chambers'>) {
      state.fallbackSource = action.payload
    },
  },
